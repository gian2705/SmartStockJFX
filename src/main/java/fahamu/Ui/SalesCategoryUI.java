...
public class SalesCategoryUI {
    ...
    private ComboBox<String> productNameChooserWhole;
    ...
    //the cash sale section
    GridPane setWholeSaleUI() {
        //for admin
        GridPane wholeSaleUI = new GridPane();
        wholeSaleUI.setPadding(new Insets(10, 10, 10, 10));
        wholeSaleUI.setId("whole_sale");
        RowConstraints row0 = new RowConstraints();
        ColumnConstraints column0 = new ColumnConstraints(300);
        ColumnConstraints column1 = new ColumnConstraints();
        row0.setVgrow(Priority.ALWAYS);
    ...
    private void setWholeSaleDataInputPaneAdmin(GridPane cashSaleUI) {
        Label dataInputTitle = new Label("WHOLE SALE");
        Label productNameLabel = new Label("Product Name :");
        Label priceLabel = new Label("Price(TZS) :");
        Label wholeSaleLabel = new Label("WholeSale");
        Label quantityLabel = new Label("Quantity :");
        ...
        gridPaneDataInput.add(dataInputTitle, 1, 0);
        gridPaneDataInput.add(productNameLabel, 0, 1);
        gridPaneDataInput.add(productNameChooserWhole, 1, 1);
        gridPaneDataInput.add(shelfLabel, 0, 2);
        gridPaneDataInput.add(shelfTextField, 1, 2);
        gridPaneDataInput.add(remainInStockLabel, 0, 3);
        gridPaneDataInput.add(remainInStockTextField, 1, 3);
        ...
        productNameChooserWhole.getEditor().setOnMouseClicked(event -> {
        ...
        productNameChooserWhole.getEditor().setOnKeyPressed(event -> {
            removeFromCart.setVisible(false);
        ...
        productNameChooserWhole.getEditor().setOnKeyReleased(event -> {
            ...
            switch (event.getCode()) {
                ...
                case BACK_SPACE: {
                    ...
                    ObservableList<String> filter = FXCollections.observableArrayList();
                    //filter the products with the word typed
                    productNameChooserWhole.getItems().clear();
                    for (String product : products) {

                        //implement not case sensitive
                        String inputString = productNameChooserWhole.getEditor().getText().toLowerCase();
                        String productStock = product.toLowerCase();

                        if (productStock.contains(inputString)) {
                            productNameChooserWhole.getItems().add(product);
                        }
                    }

                    if (productNameChooserWhole.getItems().isEmpty()) filter.add("No such item in stock");

                    if (productNameChooserWhole.getEditor().getText().isEmpty()) {
                        productNameChooserWhole.setItems(products);
                        productNameChooserWhole.show();
                    }
                ...
                default: {
                    ...
                    if (productNameChooserWhole.getItems().isEmpty()) filter1.add("No such item in stock");
        ...
        productNameChooserWhole.setOnKeyPressed(event -> {
            switch (event.getCode()) {
                case ENTER:
                    if (!productNameChooserWhole.getSelectionModel().isEmpty()) {
