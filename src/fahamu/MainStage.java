...
import static fahamu.StockCategoryUI.popupAddCategory;


class MainStage {

    private fahamu.SalesCategoryUI salesCategoryUI;
    private fahamu.PurchaseCategoryUI purchaseCategoryUI;
    private fahamu.StockCategoryUI stockCategoryUI;
    ...
    MainStage(int service, fahamu.SalesCategoryUI salesCategoryUI) {
        this.salesCategoryUI = salesCategoryUI;
        purchaseCategoryUI = new fahamu.PurchaseCategoryUI();
        stockCategoryUI = new fahamu.StockCategoryUI();
    ...
    private void setNavigationNode() {
        ...
        expensesNav.setOnMouseClicked(event -> {
            ...
            for (int i = 0; i < index; i++) {
                ...
                if (node.getId().equals("new_expenditure")) {
                    ...
                    if (node.toString().startsWith("TabPan")) {
                        rootAdmin.getChildren().remove(i);
                        rootAdmin.add(fahamu.ExpenditureCategoryUI.newExpenditureUI, 1, 1);
                        break;
        ...
        addSupplier.setOnAction(event -> fahamu.PurchaseCategoryUI.popup.show(stageAdmin));
        removeSupplier.setOnAction(event -> fahamu.PurchaseCategoryUI.popupRemoveSupplier.show(stageAdmin));
        //implement expenditure category buttons
        addExpensesCategories.setOnAction(event -> fahamu.ExpenditureCategoryUI.popup.show(stageAdmin));
        ...
        addCategoryButton.setOnAction(event -> popupAddCategory.show(stageAdmin));
        removeCategoryButton.setOnAction(event -> {
            fahamu.StockCategoryUI.categoryList.clear();
            StockCategoryData.getAllCategories();
            fahamu.StockCategoryUI.popupRemoveCategory.show(stageAdmin);
        });

        addUnitbutton.setOnAction(event -> fahamu.StockCategoryUI.popupAddUnit.show(stageAdmin));
        removeUnitButton.setOnAction(event -> {
            fahamu.StockCategoryUI.unitList.clear();
            StockCategoryData.getAllUnit();
            fahamu.StockCategoryUI.popupRemoveUnit.show(stageAdmin);
        });

        addReceipt.setOnAction(event -> fahamu.PurchaseCategoryUI.popupAddReceipt.show(stageAdmin));
        removeReceipt.setOnAction(event -> fahamu.PurchaseCategoryUI.popupRemoveReceipt.show(stageAdmin));

        addInvoice.setOnAction(event -> fahamu.PurchaseCategoryUI.popupAddInvoice.show(stageAdmin));
        removeInvoice.setOnAction(event -> fahamu.PurchaseCategoryUI.popupRemoveInvoice.show(stageAdmin));
